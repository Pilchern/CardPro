...
# NOTE: an earlier version fetched the eBay item page to recover a truncated
# title, sending a spoofed desktop-Chrome User-Agent. That is automated
# access to eBay dressed up as a browser -- against eBay's User Agreement
# (which names automated means and LLM-driven bots explicitly since Feb
# 2026) and against this project's own rule about not working around a
# marketplace's anti-automation measures. Removed; a truncated grade is
# reported as uncertain, which is what the fallback already did._TITLE_SUFFIXES_TO_STRIP = (" | eBay", " for sale online | eBay", " | eBay US")
